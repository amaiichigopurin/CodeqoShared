...
{
    public class DisplayDialogInputField : EditorWindow
    {
        ...
        /// <returns></returns>
        public static string Show(string title, string description, Action<string> onComplete, string okButton = "OK", string cancelButton = "Cancel")
        {
            ...
            var window = CreateInstance<DisplayDialogInputField>();
            ...
            window.onOKButton += () => onComplete?.Invoke(window.inputText);
        ...
        public static string ShowAITranslation(string title, string translation, Action<string> onComplete, string okButton = "OK", string cancelButton = "Cancel")
        {
            // Make sure our popup is always inside parent window, and never offscreen
            // So get caller's window size
            var maxPos = GUIUtility.GUIToScreenPoint(new Vector2(Screen.width, Screen.height));
            ...
            string ret = null;
            var window = (DisplayDialogInputField)GetWindow(typeof(DisplayDialogInputField), true, title, true);
            /*
            window.maxScreenPos = maxPos;
            window.titleContent = new GUIContent(title);
            window.description = "자동 완성 텍스트";
            window.inputText = translation;*/

            /* increase inputField height */
            window.minSize = new Vector2(600, 300);
            translation = EditorGUILayout.TextArea(translation, GUILayout.Height(400));
